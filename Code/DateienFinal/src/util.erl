...
-export([list2set/1,shuffle/1,list2string/1,slist_tointlist/1,writelist/2,readlist/1,
		 ...
		 logging/2,logstop/0,timeMilliSecond/0,logS/3,logA/3]).
...
% Synchrones loggen
logS(File, String, ArgList) ->
	{ok, IODevice} = file:open(File, [append]),
	shell:strings(false),
	io:format(IODevice, String, ArgList),
	file:close(IODevice),
	ok.

% Asynchrones loggen
logA(Datei,Inhalt,Argumente) ->
	Known = erlang:whereis(mylog),
	case Known of
		undefined ->
			PIDmylog = spawn(fun() -> logloopA(0) end),
			erlang:register(mylog,PIDmylog);
		_NotUndef -> ok
	end,
	mylog ! {Datei,Inhalt,Argumente},
	ok.
% Beendet den nebenläufigen Prozess
logstopA() ->
	Known = erlang:whereis(mylog),
...
logloopA(Y) ->
	receive
		{Datei,Inhalt,Argumente} ->
			logS(Datei,Inhalt,Argumente),
			logloopA(Y+1);
		kill ->
			true
	end.
